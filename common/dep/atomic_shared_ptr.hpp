...
public:
	template <typename T, typename... Args> static std::shared_ptr<T> make_ptr(Args &&...args) {
		return std::make_shared<T>(std::forward<Args...>(args...));
	}
	typedef std::__shared_count<std::_S_atomic> shared_count;
	typedef std::_Sp_counted_base<std::_S_atomic> counted_base;
	template <typename T> using CountedPtr = std::shared_ptr<T>;

	template <typename T> static counted_base *get_counted_base(const std::shared_ptr<T> &bar);

	static void inc_shared_count(counted_base *base, long count);

	template <typename T> static void release_shared(counted_base *base, long count);

	template <typename T> static T *get_shared_ptr(counted_base *base);

	template <typename T> static T *release_ptr(std::shared_ptr<T> &p);
...
template struct shared_ptr_internals::Rob<shared_ptr_internals::access_shared_ptr,
                                          &std::__shared_ptr<const void, std::_S_atomic>::_M_refcount>;
template struct shared_ptr_internals::Rob<shared_ptr_internals::access_base,
                                          &shared_ptr_internals::shared_count::_M_pi>;
template struct shared_ptr_internals::Rob<shared_ptr_internals::access_use_count,
                                          &shared_ptr_internals::counted_base::_M_use_count>;
template struct shared_ptr_internals::Rob<shared_ptr_internals::access_weak_count,
                                          &shared_ptr_internals::counted_base::_M_weak_count>;
template struct shared_ptr_internals::Rob<shared_ptr_internals::access_counted_ptr_ptr,
                                          &std::_Sp_counted_ptr<const void *, std::_S_atomic>::_M_ptr>;
template struct shared_ptr_internals::Rob<shared_ptr_internals::access_shared_ptr_ptr,
                                          &std::__shared_ptr<const void, std::_S_atomic>::_M_ptr>;
template struct shared_ptr_internals::Rob<shared_ptr_internals::access_refcount,
                                          &std::__shared_ptr<const void, std::_S_atomic>::_M_refcount>;

template <typename T>
inline shared_ptr_internals::counted_base *shared_ptr_internals::get_counted_base(const std::shared_ptr<T> &bar) {
	...
	// shared_ptr<T>)
	const std::shared_ptr<const void> &ptr(reinterpret_cast<const std::shared_ptr<const void> &>(bar));
	return (ptr.*fieldPtr(access_shared_ptr{})).*fieldPtr(access_base{});
}

inline void shared_ptr_internals::inc_shared_count(counted_base *base, long count) {
...
template <typename T> inline void shared_ptr_internals::release_shared(counted_base *base, long count) {
...
template <typename T> inline T *shared_ptr_internals::get_shared_ptr(counted_base *base) {
...
template <typename T> inline T *shared_ptr_internals::release_ptr(std::shared_ptr<T> &p) {
	auto res = p.get();
...
template <typename T>
inline std::shared_ptr<T> shared_ptr_internals::get_shared_ptr_from_counted_base(counted_base *base, bool inc) {
	if (!base) {
		return nullptr;
	}
	...
	newp.*fieldPtr(access_shared_ptr_ptr{}) = get_shared_ptr<const void>(base); // _M_ptr
...
template <class T> class PackedSyncPtr {
...
static_assert(std::is_pod<PackedSyncPtr<void>>::value, "PackedSyncPtr must be kept a POD type.");
static_assert(sizeof(PackedSyncPtr<void>) == 8, "PackedSyncPtr should be only 8 bytes---something is "
                                                "messed up");

} // namespace detail

template <typename T, typename CountedDetail = detail::shared_ptr_internals> class atomic_shared_ptr {
...
public:
	...
	void operator=(SharedPtr desired) /* noexcept */ { store(std::move(desired)); }
	...
	void store(SharedPtr n, std::memory_order order = std::memory_order_seq_cst) /* noexcept */ {
		auto newptr = get_newptr(std::move(n));
		auto old = ptr_.exchange(newptr, order);
		release_external(old);
	}

	SharedPtr exchange(SharedPtr n, std::memory_order order = std::memory_order_seq_cst) /* noexcept */ {
		auto newptr = get_newptr(std::move(n));
		auto old = ptr_.exchange(newptr, order);
	...
	bool compare_exchange_weak(SharedPtr &expected, const SharedPtr &n, std::memory_order success,
	...
	bool compare_exchange_weak(SharedPtr &expected, SharedPtr &&desired, std::memory_order success,
	                           std::memory_order failure) /* noexcept */ {
	...
	bool compare_exchange_strong(SharedPtr &expected, const SharedPtr &n, std::memory_order success,
	                             std::memory_order failure) /* noexcept */ {
	...
	bool compare_exchange_strong(SharedPtr &expected, SharedPtr &&desired, std::memory_order success,
	                             std::memory_order failure) /* noexcept */ {
...
private:
	...
	unsigned int get_local_count(const PackedPtr &p) const { return p.extra() & ~ALIASED_PTR; }
	...
	bool owners_eq(PackedPtr &p1, BasePtr *p2) {
		...
		if (aliased1) {
			auto p1a = CountedDetail::template get_shared_ptr_from_counted_base<T>(p1.get(), false);
	...
	SharedPtr get_shared_ptr(const PackedPtr &p, bool inc = true) const {
		...
		auto res = CountedDetail::template get_shared_ptr_from_counted_base<T>(p.get(), inc);
		if (aliased) {
			auto aliasedp = CountedDetail::template get_shared_ptr_from_counted_base<SharedPtr>(p.get());
			res = *aliasedp;
		}
		return res;
	}
