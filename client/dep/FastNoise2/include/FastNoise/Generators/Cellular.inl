...
template <typename FS>
class FS_T<FastNoise::CellularValue, FS> : public virtual FastNoise::CellularValue,
                                           public FS_T<FastNoise::Cellular, FS> {
			...
			for (int yi = 0; yi < 3; yi++) {
				...
				for (int zi = 0; zi < 3; zi++) {
					...
					for (int wi = 0; wi < 3; wi++) {
						...
						float32v invMag =
						    jitter * FS_InvSqrt_f32(FS_FMulAdd_f32(
						                 xd, xd, FS_FMulAdd_f32(yd, yd, FS_FMulAdd_f32(zd, zd, wd * wd))));
						xd = FS_FMulAdd_f32(xd, invMag, xcf);
						yd = FS_FMulAdd_f32(yd, invMag, ycf);
						zd = FS_FMulAdd_f32(zd, invMag, zcf);
...
template <typename FS>
class FS_T<FastNoise::CellularDistance, FS> : public virtual FastNoise::CellularDistance,
                                              public FS_T<FastNoise::Cellular, FS> {
	FASTSIMD_DECLARE_FS_TYPES;
...
template <typename FS>
class FS_T<FastNoise::CellularLookup, FS> : public virtual FastNoise::CellularLookup,
                                            public FS_T<FastNoise::Cellular, FS> {
	FASTSIMD_DECLARE_FS_TYPES;
	...
	float32v FS_VECTORCALL Gen(int32v seed, float32v x, float32v y) const final {
		...
		return this->GetSourceValue(mLookup, seed, cellX * float32v(mLookupFreq), cellY * float32v(mLookupFreq));
	}
	...
	float32v FS_VECTORCALL Gen(int32v seed, float32v x, float32v y, float32v z) const final {
		...
		return this->GetSourceValue(mLookup, seed - int32v(-1), cellX * float32v(mLookupFreq),
		                            cellY * float32v(mLookupFreq), cellZ * float32v(mLookupFreq));
	}
