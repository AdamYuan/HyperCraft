...
public:
	...
	static std::shared_ptr<Buffer> Create(const std::shared_ptr<Device> &device, VkDeviceSize size,
	                                      VmaAllocationCreateFlags allocation_flags, VkBufferUsageFlags buffer_usage,
	                                      VmaMemoryUsage memory_usage = VMA_MEMORY_USAGE_AUTO,
	                                      const std::vector<std::shared_ptr<Queue>> &access_queues = {});
	...
	template <typename T>
	static std::shared_ptr<Buffer> CreateStaging(const std::shared_ptr<Device> &device, const T &data,
	                                             const std::vector<std::shared_ptr<Queue>> &access_queues = {}) {
		std::shared_ptr<Buffer> ret =
	...
	inline const std::shared_ptr<Device> &GetDevicePtr() const override { return m_device_ptr; }
