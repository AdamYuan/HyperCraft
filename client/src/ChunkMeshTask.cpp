...
void ChunkTaskRunner<ChunkTaskType::kMesh>::Run(ChunkTaskPool *p_task_pool,
	...
	{
		auto [locked_chunks] = Chunk::Lock<ChunkLockType::kAllR>(neighbour_chunks);
		...
		for (InnerPos1 y = -15; y < (InnerPos1)kChunkSize + 15; ++y)
			for (InnerPos1 z = -15; z < (InnerPos1)kChunkSize + 15; ++z)
				for (InnerPos1 x = -15; x < (InnerPos1)kChunkSize + 15; ++x) {
					block::Light light = {};
					uint32_t sur_idx = GetBlockChunkSurroundIndex(x, y, z);
					light.SetTorchlight(locked_chunks[sur_idx].GetBlockFromNeighbour(x, y, z).GetLightLevel());
					light.SetSunlight(locked_chunks[sur_idx].GetSunlightFromNeighbour(x, y, z) ? 15 : 0);
			...
			for (InnerPos1 z = -15; z < (InnerPos1)kChunkSize + 15; ++z)
				for (InnerPos1 x = -15; x < (InnerPos1)kChunkSize + 15; ++x) {
					...
					if (LightAlgo::IsBorderLightInterfere(x, y, z, sunlight)) {
						if ((x == -15 || m_extend_light_buffer[chunk_xyz_extended15_to_index(InnerPos1(x - 1), y, z)]
						                         .GetSunlight() == 15) &&
						    (x == (InnerPos1)kChunkSize + 14 ||
						     m_extend_light_buffer[chunk_xyz_extended15_to_index(InnerPos1(x + 1), y, z)]
						             .GetSunlight() == 15) &&
						    (z == -15 || m_extend_light_buffer[chunk_xyz_extended15_to_index(x, y, InnerPos1(z - 1))]
						                         .GetSunlight() == 15) &&
						    (z == (InnerPos1)kChunkSize + 14 ||
						     m_extend_light_buffer[chunk_xyz_extended15_to_index(x, y, InnerPos1(z + 1))]
						             .GetSunlight() == 15) &&
						    (y == -15 || m_extend_light_buffer[chunk_xyz_extended15_to_index(x, InnerPos1(y - 1), z)]
						                         .GetSunlight() == 15) &&
						    (y == (InnerPos1)kChunkSize + 14 ||
						     m_extend_light_buffer[chunk_xyz_extended15_to_index(x, InnerPos1(y + 1), z)]
						             .GetSunlight() == 15))
							continue;
		...
		algo.PropagateLight(
		    &m_sunlight_entries,
		    [&locked_chunks](auto x, auto y, auto z) -> block::Block {
		...
		meshes = BlockMeshAlgo<BlockAlgoConfig<
		    InnerPos1, BlockAlgoBound<InnerPos1>{0, 0, 0, kChunkSize, kChunkSize, kChunkSize}, kBlockAlgoSwizzleYZX>>{}
		             .Generate(
		                 [&locked_chunks](auto x, auto y, auto z) -> block::Block {
			                 return locked_chunks[GetBlockChunkSurroundIndex(x, y, z)].GetBlockFromNeighbour(x, y, z);
		                 },
