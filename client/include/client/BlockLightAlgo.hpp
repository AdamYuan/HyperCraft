...
private:
	...
	static inline constexpr auto kDefaultSetOutOfBoundLightFunc = [](auto, auto, auto, block::LightLvl) -> void {};

public:
	template <bool VerticalLight = false>
	static inline constexpr bool IsBorderLightInterfere(T x, T y, T z, block::LightLvl lvl) {
		...
		if constexpr (VerticalLight) {
			dist += y < kBound.min_y ? -(uint32_t)y + (uint32_t)kBound.min_y : 0;
		} else {
	...
	template <bool VerticalLight = false, typename GetBlockFunc, typename GetLightFunc, typename SetLightFunc,
	          typename SetOutOfBoundLightFunc = decltype(kDefaultSetOutOfBoundLightFunc)>
	inline void PropagateLight(Queue &&entries, GetBlockFunc get_block_func, GetLightFunc get_light_func,
	                           ...
	                           SetOutOfBoundLightFunc set_out_of_border_light_func = kDefaultSetOutOfBoundLightFunc) {
		...
		while (!m_queue.empty()) {
			...
			for (block::BlockFace f = 0; f < 6; ++f) {
				Entry nei = e;
				if constexpr (VerticalLight)
					nei.lvl -= f != block::BlockFaces::kBottom;
				block::BlockFaceProceed(glm::value_ptr(nei.pos), f);

				if (!in_bound_bordered(nei.pos.x, nei.pos.y, nei.pos.z)) {
					set_out_of_border_light_func(nei.pos.x, nei.pos.y, nei.pos.z, nei.lvl);
					continue;
				}

				if constexpr (Border > 0)
					if (!IsBorderLightInterfere<VerticalLight>(nei.pos.x, nei.pos.y, nei.pos.z, nei.lvl))
						continue;
	...
	template <bool VerticalLight = false, typename GetBlockFunc, typename GetLightFunc, typename SetLightFunc,
	          typename SetOutOfBoundLightFunc = decltype(kDefaultSetOutOfBoundLightFunc)>
	inline void RemoveLight(Queue &&entries, GetBlockFunc get_block_func, GetLightFunc get_light_func,
	                        SetLightFunc set_light_func,
	                        SetOutOfBoundLightFunc set_out_of_border_light_func = kDefaultSetOutOfBoundLightFunc) {
		...
		while (!m_queue.empty()) {
			...
			for (block::BlockFace f = 0; f < 6; ++f) {
				...
				if (nei.lvl < get_light_func(nei.pos.x, nei.pos.y, nei.pos.z) &&
				    get_block_func(nei.pos.x, nei.pos.y, nei.pos.z).GetIndirectLightPass()) {
					set_light_func(nei.pos.x, nei.pos.y, nei.pos.z, 0);
					if (nei.lvl > 1)
						m_queue.push(nei);
				}
			}
