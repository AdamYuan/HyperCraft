...
public:
	inline void Initialize(myvk_rg::BufferInput mesh_info_buffer, myvk_rg::BufferInput camera_buffer,
		...
		opaque_draw_cmd_buffer->SetSize(ChunkMeshPool::kMaxClusters * ChunkMeshPool::kMaxMeshesPerCluster *
		                                sizeof(VkDrawIndexedIndirectCommand));
		opaque_draw_count_buffer->SetSize(ChunkMeshPool::kMaxClusters * sizeof(uint32_t));
		...
		trans_draw_cmd_buffer->SetSize(ChunkMeshPool::kMaxClusters * ChunkMeshPool::kMaxMeshesPerCluster *
		                               sizeof(VkDrawIndexedIndirectCommand));
		trans_draw_count_buffer->SetSize(ChunkMeshPool::kMaxClusters * sizeof(uint32_t));
...
#include <client/shader/chunk_culling.comp.u32>
	...
	inline void CmdExecute(const myvk::Ptr<myvk::CommandBuffer> &command_buffer) const final {
		...
		std::fill(opaque_draw_count_mapped, opaque_draw_count_mapped + ChunkMeshPool::kMaxClusters, 0u);
		std::fill(trans_draw_count_mapped, trans_draw_count_mapped + ChunkMeshPool::kMaxClusters, 0u);

		for (const auto &cluster : *m_p_prepared_clusters) {
			uint32_t pc_data[] = {cluster->GetLocalMeshCount(),
			                      cluster->GetClusterOffset() * ChunkMeshPool::kMaxMeshesPerCluster,
